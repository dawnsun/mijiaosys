...
CREATE TABLE `member_user` (
  ...
  `member_card_number` varchar(64) DEFAULT NULL COMMENT '会员卡号',
...
-- ----------------------------
-- Table structure for `recommender_income`
-- ----------------------------
DROP TABLE IF EXISTS `recommender_income`;
CREATE TABLE `recommender_income` (
  ...
  `gmt_withdrawals` datetime DEFAULT NULL COMMENT '提现时间',
  `gmt_create` datetime NOT NULL COMMENT '创建时间',
  `gmt_modified` datetime NOT NULL COMMENT '修改时间',
  `creator` varchar(32) NOT NULL COMMENT '创建人',
  `modifier` varchar(32) NOT NULL COMMENT '修改人',
  `is_delete` tinyint(1) NOT NULL DEFAULT '0' COMMENT '0-有效 1-删除',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='推荐收入表';

-- ----------------------------
-- Records of recommender_income
-- ----------------------------
